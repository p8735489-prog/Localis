...
class SettingsRepository(private val context: Context) {
    ...
    suspend fun setTorExitCountry(country: String) {
        context.settingsDataStore.edit { prefs -> prefs[Keys.TOR_EXIT_COUNTRY] = country.trim().lowercase().take(2) }
    }

    suspend fun setTorCustomConfig(config: String) {
        context.settingsDataStore.edit { prefs -> prefs[Keys.TOR_CUSTOM_CONFIG] = config.take(8192) }
    }
